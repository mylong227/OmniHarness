...
test('combination: 边界与对称性', () => {
  ...
  assert.strictEqual(combination(5, 3), 10);
  assert.strictEqual(combination(5, 6), 0); // k>n
});
...
test('computePassK: 全通过 → 所有 k 均为 1', () => {
  ...
  const r = computePassK(outcomes, 3);
...
test('computePassK: 全失败 → 所有 k 均为 0', () => {
  ...
  const r = computePassK(outcomes, 2);
...
test('passKGate: 通过率达标 → passed', () => {
  ...
  const g = passKGate(s, { minPassRate: 0.6 });
...
test('passKGate: 通过率不达标 → 列出失败', () => {
  ...
  const g = passKGate(s, { minPassRate: 0.8 });
...
test('passKGate: Pass@k 阈值', () => {
  ...
  const s = summarizePassK([[true, true, false, false]], 2);
  assert.ok(Math.abs(s.passAtK[0]! - 0.5) < 1e-9);
  assert.ok(Math.abs(s.passAtK[1]! - (1 - 1 / 6)) < 1e-9);
  const g = passKGate(s, { minPassK: [{ k: 2, threshold: 0.9 }] });
  assert.strictEqual(g.passed, false);
  const ok = passKGate(s, { minPassK: [{ k: 2, threshold: 0.7 }] });
