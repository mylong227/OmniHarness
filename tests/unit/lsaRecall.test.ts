...
import { trainLsa, lsaQuery } from '../../src/context/lsaEngine.js';
...
describe('LSA 潜语义召回', () => {
  it('相同种子训练结果确定性一致（可复现）', () => {
    const c = buildCorpus();
    const m1 = trainLsa(c, 16, 42);
    const m2 = trainLsa(c, 16, 42);
  ...
  it('query 返回按分数降序排列的命中', () => {
    const c = buildCorpus();
    const m = trainLsa(c, 16, 42);
    const res = lsaQuery(m, 'sandbox policy', 3);
  ...
  it('同模型同查询幂等（结果可复现）', () => {
    const c = buildCorpus();
    const m = trainLsa(c, 16, 42);
    const r1 = lsaQuery(m, 'evaluate decision');
    const r2 = lsaQuery(m, 'evaluate decision');
    assert.deepStrictEqual(r1, r2);
  });

  it('不同种子产生不同投影基（随机性生效）', () => {
    const c = buildCorpus();
    const m1 = trainLsa(c, 16, 1);
    const m2 = trainLsa(c, 16, 999);
    let differs = false;
