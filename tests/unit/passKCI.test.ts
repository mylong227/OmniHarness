...
import { Bootstrap } from '../../src/eval/bootstrap.js';

test('mulberry32: 同种子同序列，异种子异序列', () => {
  const a = Bootstrap.mulberry32(12345);
  const b = Bootstrap.mulberry32(12345);
  const c = Bootstrap.mulberry32(54321);
  const seqA = [a(), a(), a()];
  const seqB = [b(), b(), b()];
  const seqC = [c(), c(), c()];
...
test('percentile: 最近秩取值与边界夹取', () => {
  ...
  assert.strictEqual(Bootstrap.percentile(s, 0), 0);
  assert.strictEqual(Bootstrap.percentile(s, 1), 9);
  assert.strictEqual(Bootstrap.percentile(s, -1), 0); // 夹取下界
  assert.strictEqual(Bootstrap.percentile(s, 2), 9); // 夹取上界
  assert.strictEqual(Bootstrap.percentile([], 0.5), 0);
});

test('bootstrapInterval: 同种子可复现；空输入退化', () => {
  const items = [1, 2, 3, 4, 5, 6, 7, 8];
  const stat = (rs: readonly number[]): number => rs.reduce((s, v) => s + v, 0) / rs.length;
  const r1 = Bootstrap.bootstrapInterval(items, stat, { seed: 7, rounds: 500 });
  const r2 = Bootstrap.bootstrapInterval(items, stat, { seed: 7, rounds: 500 });
  assert.deepStrictEqual(r1, r2);
  assert.ok(r1.lo <= r1.mean && r1.mean <= r1.hi);
  const empty = Bootstrap.bootstrapInterval<number>([], stat, { rounds: 100 });
...
test('bootstrapPassK: 点估计等于 computePassK，且区间包住点估计', () => {
  ...
  const report = PassK.bootstrapPassK(outcomes, 2);
  const point = PassK.computePassK(outcomes, 2);
...
test('bootstrapPassK: 全通过 ⇒ Pass@k 区间恒为 1', () => {
  const report = PassK.bootstrapPassK(
    [
      [true, true],
      [true, true, true],
    ],
...
test('passKGateWithCI: 全通过且阈值达标 ⇒ passed', () => {
  ...
  const g = PassK.passKGateWithCI(report, {
    minPassRate: 0.9,
    minPassK: [{ k: 2, threshold: 0.9 }],
  });
...
test('passKGateWithCI: 区间跨阈值 ⇒ inconclusive（fail-closed）', () => {
  ...
  const report = PassK.bootstrapPassK(outcomes, 1);
  const g = PassK.passKGateWithCI(report, { minPassRate: 0.5 });
  assert.strictEqual(g.passed, false);
...
test('T4.7 验收：同一输入重复判定 20 次结果完全稳定', () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const g = PassK.passKGateWithCI(PassK.bootstrapPassK(outcomes, 3), {
      minPassRate: 0.4,
...
test('T4.7 验收：bootstrapPassK 两次调用结果逐字段一致', () => {
  ...
  assert.deepStrictEqual(PassK.bootstrapPassK(outcomes, 3), PassK.bootstrapPassK(outcomes, 3));
  assert.deepStrictEqual(
    PassK.summarizePassK(outcomes, 3),
    PassK.bootstrapPassK(outcomes, 3).summary,
  );
});
