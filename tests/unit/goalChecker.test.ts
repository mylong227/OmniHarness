...
describe('parseAchieved', () => {
  it('显式 YES 视为达成', () => {
    assert.strictEqual(GoalChecker.parseAchieved('YES'), true);
    assert.strictEqual(GoalChecker.parseAchieved('YES 已完成'), true);
  });

  it('显式 NO 视为未达成', () => {
    assert.strictEqual(GoalChecker.parseAchieved('NO'), false);
    assert.strictEqual(GoalChecker.parseAchieved('NO 还没做完'), false);
  });

  it('含 YES 且无 NO 视为达成（宽松）', () => {
    assert.strictEqual(GoalChecker.parseAchieved('目标 yes 了'), true);
  });

  it('含 YES 但也被 NO 修饰时保守判未达成', () => {
    assert.strictEqual(GoalChecker.parseAchieved('not yes'), false);
    assert.strictEqual(GoalChecker.parseAchieved('yes? no, 还差一步'), false);
  });

  it('无 YES/NO 关键词时保守判未达成（避免过早停止）', () => {
    assert.strictEqual(GoalChecker.parseAchieved('目标仍在推进中'), false);
    assert.strictEqual(GoalChecker.parseAchieved('The goal is now complete'), false);
  });
});
