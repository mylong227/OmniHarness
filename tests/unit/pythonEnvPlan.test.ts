...
test('PythonEnvPlan.steps：仓库本体恒为首步；extras 紧随其后', () => {
  ...
  assert.deepEqual([...ArrayAt.at(steps, 0).args], ['-e', '.']);
  assert.deepEqual([...ArrayAt.at(steps, 1).args], ['-e', '.[test]']);
...
test('PythonEnvPlan.steps：pytest 已存在 ⇒ 不生成兜底安装步（绝不覆盖仓库 pin）', () => {
  const steps = PythonEnvPlan.steps({ pins: [], pytestPresent: true });
  assert.ok(
    !steps.some((s) => s.args.length === 1 && ArrayAt.at(s.args, 0) === 'pytest'),
...
test('PythonEnvPlan.steps：pytest 缺失 ⇒ 兜底步必为末步且仅装 pytest', () => {
  ...
  const last = ArrayAt.at(steps, steps.length - 1);
  assert.deepEqual([...last.args], ['pytest']);
  // 顺序不变量：已 pinned 测试依赖 → 额外约束 → 兜底 pytest
  const reqIdx = steps.findIndex((s) => ArrayAt.at(s.args, 0) === '-r');
  const pinIdx = steps.findIndex((s) => ArrayAt.at(s.args, 0) === 'Werkzeug<3');
