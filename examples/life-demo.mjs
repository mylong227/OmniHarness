...
const presets = {
  random: (r, c) =>
    Array.from({ length: r }, () => Array.from({ length: c }, () => Math.random() < 0.28)),
  ...
  pulsar: (r, c) => {
    ...
    for (const [a, b] of [
      ...
      for (let y = a; y <= b; y++) add(y, [2, 7, 12]);
    // 垂直段：列 2..4 与 8..10，在行 2、7、12 上
    for (const [a, b] of [
      [2, 4],
      [8, 10],
    ])
      for (let y = 2; y <= 12; y += 5) for (let x = a; x <= b; x++) add(y, [x]);
    // 居中放进画布
    const oy = Math.floor((r - 15) / 2);
    const ox = Math.floor((c - 15) / 2);
    for (let y = 0; y < 15; y++)
      for (let x = 0; x < 15; x++) if (core[y][x]) g[oy + y][ox + x] = true;
    return g;
  },
...
function render(g, gen, alive) {
  const rows = g.map((row) => row.map((v) => (v ? '\x1b[32m█\x1b[0m' : ' ')).join('')).join('\n');
