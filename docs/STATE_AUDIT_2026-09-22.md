...
| **发布状态**         | ✅ 已收口       | 在途批次按 changeset 分笔入库（`76b186f` / `5d5646a` / `3ed7c11` + 本笔 docs），工作树清零；20 → 23 个 changeset 待消费                     |
...
| 提交数    | 317（分支 `main`）                                                                                             |
| 版本      | `0.1.0`（未发布，`Unreleased` + 20 changesets 待消费）                                                         |
| 工作树    | 13 个已跟踪文件修改（+552 / −39），15 个未跟踪条目（3 changeset + 2 新 src + 5 新测试 + 2 新 eval + 2 报告）   |
...
- README 自我宣称的规模（441 TS / 5.5 万行）已**过期**，实测为 **560 TS / 65,786 行**（已在收尾中同步，见 §8）。
