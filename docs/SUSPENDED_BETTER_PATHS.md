...
> 的更优解方法已确定（上游 Modal 云执行，零本地 docker），但执行仍须你侧 cloud 凭证**。
...
| **B1** 官方 500 Verified | 本机 docker + HF 数据集    | 上游 Modal 云执行 `--modal`（gVisor 隔离）                  | 一致（官方同款 harness） | 更高（~7min/500 题）  | ⏸️ 方法确定，待 cloud 凭证                   |
...
## 四、B1：官方 500 Verified → 上游 Modal 云执行
...
**官方 500 Verified 的更优解**：SWE-bench 官方 FAQ 明确「不能脱离 Docker 跑评测」，
但官方 harness 已 **upstream 支持 Modal 云执行**（`--modal`）：在 Modal 的
gVisor 隔离容器里跑 500 题，官方文档称 **~7 分钟**完成，零本地 docker、零本机 HF 数据集下载。

- **保真度**：与官方同款 harness + 同款评测脚本，口径一致（甚至更标准）。
- **效率**：~7min/500 题，远高于「本机逐一起 docker 容器」。
- **参考开源**：SWE-bench 官方 `swebench` harness 的 Modal 集成（`--modal` flag）。

**未决**：执行 Modal 仍需你侧 cloud 凭证（沙箱代理 502 + ghproxy Basic 认证挂死，无法在
agent shell 跑）。方法已确定，待凭证解锁后即可在更优路径上出官方 Verified 真实分数。
