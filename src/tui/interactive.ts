 ...
 */
export async function startInteractive(opts: InteractiveOptions): Promise<void> {
  ...
  out.write(`${renderStatusLine('就绪', '输入消息开始；exit/quit 退出')}\n`);
  try {
    while (true) {
      ...
      out.write(clearLine());
      try {
        await renderStream(opts.send(trimmed), out);
      } catch (err) {
        out.write(
          `${renderEventLine({ kind: 'error', text: `处理失败: ${(err as Error).message}` })}\n`,
        );
      }
    }
  } finally {
    if (opts.rl === undefined) rl.close();
    out.write(`${renderStatusLine('已退出')}\n`);
  }
}
