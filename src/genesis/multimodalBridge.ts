...
/**
 * 多模态桥接器：原模块级纯函数归拢为 `MultimodalBridge` 静态方法族，
...
export class MultimodalBridge {
  ...
  static modelMessageToModality(msg: ModelMessage): Modality<unknown> {
    ...
    if (msg.images && msg.images.length > 0) {
      ...
      let acc: Modality<unknown> = MultimodalBridge.imageModality(first) as Modality<unknown>;
      for (let i = 1; i < msg.images.length; i++) {
        const im = msg.images[i];
        if (im !== undefined)
          acc = fuseModality(acc, MultimodalBridge.imageModality(im)) as Modality<unknown>;
      }
  ...
  private static imageModality(img: {
    url?: string;
    data?: string;
    mediaType?: string;
  }): Modality<Uint8Array> {
    ...
    if (img.data !== undefined) {
      bytes = MultimodalBridge.base64ToBytes(img.data);
    } else if (img.url !== undefined) {
      bytes = MultimodalBridge.asciiToBytes(img.url);
    } else {
  ...
  private static asciiToBytes(s: string): Uint8Array {
...
// ---- 门面兼容：保留原导出名 ----
export const modelMessageToModality = MultimodalBridge.modelMessageToModality;
export const crossModalAlign = MultimodalBridge.crossModalAlign;
export const registerCrossModal = MultimodalBridge.registerCrossModal;
export const modalitySignature = MultimodalBridge.modalitySignature;
