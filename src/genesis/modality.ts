...
export function encodeText(s: string): Modality<string> {
  return { kind: 'text', data: s, features: textFeatures(s) };
}
...
export function encodeImage(
  ...
  return { kind: 'image', data: bytes, features: imageFeatures(bytes, width, height) };
}

/** 特征签名（确定性，用于融合时的规范排序以保交换律）。 */
function featureSig(f: ReadonlyArray<number>): string {
...
export function fuseModality<A, B>(a: Modality<A>, b: Modality<B>): Modality<[A, B]> {
  const ka = `${a.kind}#${featureSig(a.features)}`;
  const kb = `${b.kind}#${featureSig(b.features)}`;
