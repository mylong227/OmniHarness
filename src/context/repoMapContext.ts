 ...
 *
 * OOP 收口（2026-09-10）：原模块级 `cache` / `semanticCache` 两张可变 Map 收为
 * `RepoMapContextEngine` 实例字段，全部检索逻辑收为方法，消除模块级可变全局。
 * 为不触碰热区文件 `src/core/stepRunner.ts`（Agent Loop V2 会话活跃）的调用点，仍导出同名
 * 函数作为单例 `repoMapContextEngine` 的薄包装；新调用方应直接用引擎实例。
 */
...
/**
 * repo-map 检索引擎（OOP 收口，2026-09-10）。
 *
 * 原 `repoMapContext.ts` 的两张模块级可变 Map（`cache` / `semanticCache`）收为实例字段，
 * 全部检索逻辑收为方法，消除模块级可变全局。进程级共享单例见文件底部 `repoMapContextEngine`。
 */
export class RepoMapContextEngine {
  /** 按 workspace 根路径缓存的语料（TTL 失效）。 */
   ...
   */
  private readonly semanticCache = new Map<string, Promise<SemanticIndex | null>>();
  ...
  private getCached(root: string): IndexedCorpus | null {
    const now = Date.now();
  ...
  public getRepoMapContext(root: string, q: string, opts: RepoMapContextOptions = {}): string | null {
    ...
    const corpus = this.getCached(root);
    if (corpus === null) {
      return null;
    }
    try {
   ...
   */
  private async getSemanticIndex(
    root: string,
    corpus: IndexedCorpus,
    ...
    const key = this.semanticKey(root, chunkRecall, fullFileDoc, docMode);
    const existing = this.semanticCache.get(key);
    ...
    const promise = (async (): Promise<SemanticIndex | null> => {
      try {
        ...
        const fileDocs = this.buildFileDocTexts(corpus, docMode, fullFileDoc);
    ...
    this.semanticCache.set(key, promise);
  ...
  public async getHybridRepoMapContext(
    ...
    try {
      ...
      const idx = await this.getSemanticIndex(root, corpus, embedding, chunkRecall, fullFileDoc, docMode);
      ...
      const rrfK = this.numericKnob(opts.rrfK, process.env.OMNI_RRF_K, 60, 1);
      // 默认 1.0（等权）：33 条真实查询扫描下 w=1.0 → +7.7pp，w=0.5 仅 +2.3pp（详见接口注释）。
      const semWeight = this.numericKnob(opts.semWeight, process.env.OMNI_SEM_WEIGHT, 1, 0);
      ...
      const bm25Floor = this.numericKnob(opts.bm25Floor, process.env.OMNI_BM25_FLOOR, 0, 0);
      ...
      const graphWeight = this.numericKnob(opts.graphWeight, process.env.OMNI_GRAPH_WEIGHT, 1, 0);
  ...
  public clear(root?: string): void {
    if (root === undefined) {
      ...
      for (const k of [...this.semanticCache.keys()]) {
        if (k.slice(k.indexOf('|') + 1) === root) {
          this.semanticCache.delete(k);
        }
      }
