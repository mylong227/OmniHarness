...
/**
 * RankVetoOverlap —— 由本文件原顶层函数归并而来（每个方法对应一个原函数，语义与签名逐字保留）。
 */
export class RankVetoOverlap {
  /**
   ...
   */
  public static jaccardOverlap(a: readonly string[], b: readonly string[]): number {
    const sa = new Set(a);
    const sb = new Set(b);
   ...
   */
  public static meanPairwiseJaccard(lists: readonly (readonly string[])[]): number | null {
    if (lists.length < 2) return null;
    let sum = 0;
    let n = 0;
    for (let i = 0; i < lists.length; i++) {
      for (let j = i + 1; j < lists.length; j++) {
        sum += RankVetoOverlap.jaccardOverlap(ArrayAt.at(lists, i), ArrayAt.at(lists, j));
        n++;
      }
    }
