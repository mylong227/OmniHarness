...
function buildTfIdf(corpus: IndexedCorpus): {
  vocab: string[];
...
function orthonormalize(Y: Float64Array, m: number, r: number): Float64Array {
...
function jacobiEigen(A: Float64Array, k: number): { values: Float64Array; vectors: Float64Array } {
...
export function trainLsa(corpus: IndexedCorpus, k = 64, seed = 1234567): LsaModel {
  const { vocab, termIndex, cols, vals, n } = buildTfIdf(corpus);
  const m = vocab.length;
  const r = k;

  // 随机 SVD：Y = A Ω，Q = orth(Y)，再做一次幂迭代增强。
  const rand = rng(seed);
  let Omega = new Float64Array(n * r);
  for (let i = 0; i < Omega.length; i++) Omega[i] = rand() * 2 - 1;
  let Y = sparseMatMul(cols, vals, Omega, m, n, r);
  Y = sparseMatMul(cols, vals, Y, m, n, r); // 一次幂迭代
  const Q = orthonormalize(Y, m, r);
  ...
  const { values, vectors: Vc } = jacobiEigen(C, r);
