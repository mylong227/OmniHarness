...
export function prefixReuse(cached: string, incoming: string): number {
  ...
  return commonPrefixLength(cached, incoming) / cached.length;
}
...
export function buildStablePrompt(
  ...
  for (const segment of ordered) {
    const body = options.scrub === true ? scrubVolatile(segment.text) : segment.text;
    parts.push(`\n## ${segment.key}\n${body}`);
  }
  return parts.join('');
}
...
function hexDigits(random: () => number, digits: number): string {
  let out = '';
...
export function injectVolatile(text: string, seed: number): string {
  ...
  const uuid = `${hexDigits(random, 8)}-${hexDigits(random, 4)}-${hexDigits(random, 4)}-${hexDigits(random, 4)}-${hexDigits(random, 12)}`;
...
export function jitterSegments(
  ...
  return reorderDeterministic(segments, seed).map((segment) => ({
    ...segment,
    text: injectVolatile(segment.text, seed),
  }));
}
...
export function measurePrefixStability(
  ...
  const build = (input: readonly PromptSegment[]): string =>
    buildStablePrompt(input, { scrub: canonical });
  const count = Math.max(1, variantCount);
  const base = build(jitterSegments(segments, 1));
  let sum = 0;
  let min = 1;
  for (let i = 1; i <= count; i += 1) {
    const reuse = prefixReuse(base, build(jitterSegments(segments, i)));
    sum += reuse;
    if (reuse < min) {
      min = reuse;
    }
