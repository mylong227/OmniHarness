...
/**
 * IpAddress —— 由本文件原顶层函数归并而来（每个方法对应一个原函数，语义与签名逐字保留）。
 */
export class IpAddress {
   ...
   */
  public static ipv4ToInt(ip: string): number | null {
   ...
   */
  public static isPrivateIpv4(ip: string, cidrs: readonly (readonly [string, number])[]): boolean {
    const value = IpAddress.ipv4ToInt(ip);
    if (value === null) {
      return true;
    }
    for (const [base, bits] of cidrs) {
      const baseValue = IpAddress.ipv4ToInt(base);
      if (baseValue === null) {
        continue;
      }
   ...
   */
  public static expandTrailingIpv4(ip: string): string {
   ...
   */
  public static parseIpv6Groups(ip: string): number[] | null {
   ...
   */
  public static embeddedIpv4(ip: string): string | null {
    const groups = IpAddress.parseIpv6Groups(IpAddress.expandTrailingIpv4(ip));
    if (groups === null) {
      return null;
    }
   ...
   */
  public static isPrivateIpv6(ip: string, cidrs: readonly (readonly [string, number])[]): boolean {
    const lower = ip.toLowerCase().replace(/^\[|\]$/g, '');
    if (lower === '::1' || lower === '::') {
      return true;
    }
    const embedded = IpAddress.embeddedIpv4(lower);
    if (embedded !== null) {
      return IpAddress.isPrivateIpv4(embedded, cidrs);
    }
    const groups = IpAddress.parseIpv6Groups(lower);
    if (groups === null) {
