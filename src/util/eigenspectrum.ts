 ...
 */
export function eigenSpectrum(text: string, bins = 257): Spectrum {
  ...
  const sm = gaussianSmooth(raw, 1);
  return { bins, values: normalize(sm) };
}
...
export function spectrumFromValues(values: readonly number[], bins = values.length): Spectrum {
  ...
  return { bins, values: normalize(arr) };
}
