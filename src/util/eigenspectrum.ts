...
export class EigenSpectrum {
  ...
  private gaussianSmooth(values: readonly number[], sigma: number): number[] {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = 0; j < n; j++) {
        const d = i - j;
        s += ArrayAt.at(values, j) * Math.exp(-(d * d) * inv);
      }
      out[i] = s;
    }
    return out;
  ...
  public resonance(a: Spectrum, b: Spectrum): number {
    ...
    let dot = 0;
    for (let i = 0; i < n; i++) dot += ArrayAt.at(a.values, i) * ArrayAt.at(b.values, i);
    return dot;
  }
   ...
   * （模拟振动板本征模），高斯平滑（σ=1，赋予频响带宽），L2 归一化。
   */
  public static eigenSpectrum(text: string, bins = RESONANCE_BINS): Spectrum {
