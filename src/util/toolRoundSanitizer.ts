...
/**
 * ToolRoundSanitizer —— 由本文件原顶层函数归并而来（每个方法对应一个原函数，语义与签名逐字保留）。
 */
export class ToolRoundSanitizer {
  /**
   ...
   */
  public static sanitizeToolRounds(messages: readonly ModelMessage[]): ModelMessage[] {
    // 1) 收集"合法 toolCallId"：必须紧邻某 assistant.tool_calls 出现
    const validIds = new Set<string>();
    for (let i = 0; i < messages.length; i++) {
      const m = ArrayAt.at(messages, i);
