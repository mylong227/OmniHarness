...
export function runDoctor(opts: DoctorOptions = {}): DoctorReport {
  ...
  // ② 配置文件：存在且 JSON 合法
  const config = checkConfig(opts.configPath, workspaceRoot, issues);

  // ③ 沙箱后端可用性
  const sandbox = checkSandbox(issues);
  ...
  if (existsSync(manifestPath)) {
    try {
      ...
      issues.push(`权限清单 JSON 非法: ${manifestPath} (${messageOf(error)})`);
    }
  }
...
function checkConfig(
  ...
  const path = configPath ?? findConfig(workspaceRoot);
...
function checkSandbox(issues: string[]): SandboxStatus {
  const bwrap = detectCommand('bwrap');
  const sandboxExec = detectCommand('sandbox-exec');
  // 不再仅用 process.platform 误报：RestrictedToken 真实可用需进程具备创建受限令牌的特权
  // （非管理员 Windows 上 Rust 侧 available() 实测为 false）。此处以提权探测为代理，与运行时一致。
  const restrictedToken = isElevated();
 ...
 */
export function isElevated(runProbe: () => void = defaultElevationProbe): boolean {
...
function defaultElevationProbe(): void {
