...
import { requireConcurrencyLimit } from '../util/concurrencyLimiter.js';
 ...
 */
export function requireWorkflowConcurrency(requested: unknown, fallback: number): number {
  if (requested === undefined) {
    return fallback;
  }
  try {
    return requireConcurrencyLimit(requested, 'maxConcurrency');
  } catch (error) {
