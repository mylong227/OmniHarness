...
function resolve(opts?: MoireOptions): Resolved {
  return {
    n: opts?.fieldSize ?? 64,
...
export function capabilityFieldOf(skill: Skill, n: number): number[][] {
  ...
  const seed = hashText(`${skill.name}\u0000${skill.description}\u0000${skill.instructions}`);
...
function rotateSample(B: number[][], n: number, theta: number): number[][] {
...
/** 可分离均值模糊（低通），半径 r。 */
function boxBlur(P: number[][], n: number, r: number): number[][] {
  const tmp: number[][] = [];
  for (let y = 0; y < n; y++) {
    const row: number[] = [];
    for (let x = 0; x < n; x++) {
...
export function emergenceAt(
  ...
  const R = rotateSample(B, n, theta);
  let mean = 0;
  const P: number[][] = [];
  for (let y = 0; y < n; y++) {
    const row: number[] = [];
...
function synthesizeInstructions(a: Skill, b: Skill): string {
  return [
...
export function composeByTwist(a: Skill, b: Skill, opts?: MoireOptions): Skill {
  const { n, blurR, step, minT, maxT, floor } = resolve(opts);
  ...
  for (let deg = minT; deg <= maxT; deg += step) {
    const e = emergenceAt(fa, fb, n, (deg * Math.PI) / 180, blurR);
  ...
  // 取 θ* 处的乘积场作为复合能力场
  const R = rotateSample(fb, n, (best.theta * Math.PI) / 180);
  const field: number[] = [];
  ...
  return {
    ...
    instructions: synthesizeInstructions(a, b),
