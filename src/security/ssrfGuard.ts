...
export class SsrfGuard {
  ...
  public inspectHost(host: string, options: SsrfOptions = {}): SsrfVerdict {
    ...
    const embedded = isIP(lower) === 6 ? IpAddress.embeddedIpv4(lower) : null;
    if (
    ...
    if (version === 4) {
      ...
      return IpAddress.isPrivateIpv4(lower, policy.ipv4Blocks)
    ...
    if (version === 6) {
      ...
      return IpAddress.isPrivateIpv6(lower, policy.ipv4Blocks)
...
*/
  ...
  public static defaultSsrfOptions(): SsrfOptions {
   ...
   */
  public static ssrfOptionsFor(policy?: SsrfPolicy): SsrfOptions {
    return { ...ssrfGuard.defaultOptions(), policy: policy ?? DEFAULT_SSRF_POLICY };
  }

  /** 纯字面量判定（不发起网络请求）。 */
  public static inspectHost(host: string, options: SsrfOptions = {}): SsrfVerdict {
   ...
   * URL 非法或协议非 HTTP(S) 一律按拦截处理（fail-closed）。
   */
  public static inspectUrl(rawUrl: string, options: SsrfOptions = {}): SsrfVerdict {
   ...
   * 解析失败、协议非 http/https、URL 非法一律按拦截处理——宁可拒绝也不放行。
   */
  public static async assertNotSsrf(rawUrl: string, options: SsrfOptions = {}): Promise<void> {
