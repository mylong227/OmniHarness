...
const IPV4_BLOCKS: readonly (readonly [string, number])[] = [
  ['0.0.0.0', 8], // 本网络
  ['10.0.0.0', 8], // 私有
  ['100.64.0.0', 10], // CGNAT
  ['127.0.0.0', 8], // 环回
...
export function inspectHost(host: string, options: SsrfOptions = {}): SsrfVerdict {
  ...
  if (options.allowMetadata !== true && METADATA_HOSTS.has(lower)) {
...
export function inspectUrl(rawUrl: string, options: SsrfOptions = {}): SsrfVerdict {
  ...
  return inspectHost(url.hostname, options);
}
...
export async function assertNotSsrf(
  ...
  const verdict = inspectHost(url.hostname, options);
  if (verdict.blocked) {
  ...
  if (options.resolveDns === true && isIP(url.hostname) === 0) {
    try {
      ...
      for (const entry of resolved) {
        const hostVerdict = inspectHost(entry.address, options);
