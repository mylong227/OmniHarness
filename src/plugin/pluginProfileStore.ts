...
export class PluginProfileStore {
  ...
  public save(profile: PluginProfile): string {
    ...
    const id = PluginProfileStore.sanitizeProfileName(profile.name);
    const dir = this.dir();
   ...
   */
  public static mergeProfileConfig(base: FileConfig, profile: PluginProfile): FileConfig {
   ...
   */
  public static sanitizeProfileName(name: string): string {
    const id = name
      .toLowerCase()
      .replace(/[^a-z0-9_-]+/g, '-')
      .replace(/^-+|-+$/g, '');
  ...
  public static async applyProfile(
    ...
    if (missing.length > 0) {
      throw new Error(
        `无法激活 profile "${profile.name}"：以下插件无法解析：${missing.join(', ')}`,
      );
    }
    ...
    const activated: string[] =
      toLoad.length === 0
        ? []
        : await PluginLoader.loadInstalledPlugins(manager, pluginsDir, options?.onError, toLoad);
