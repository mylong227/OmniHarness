...
/**
 * Zip —— 由本文件原顶层函数归并而来（每个方法对应一个原函数，语义与签名逐字保留）。
 */
export class Zip {
  /**
   * @beta
   ...
   */
  public static crc32(buf: Buffer): number {
    let c = 0xffffffff;
   ...
   */
  public static zipStore(entries: readonly ZipEntry[]): Buffer {
    const chunks: Buffer[] = [];
    const central: Buffer[] = [];
    let offset = 0;
    for (const entry of entries) {
      const nameBuf = Buffer.from(entry.name, 'utf8');
      const crc = Zip.crc32(entry.data);
   ...
   */
  public static unzip(buffer: Buffer): ZipEntry[] {
    let eocd = -1;
