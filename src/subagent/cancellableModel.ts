...
/**
 * CancellableModel —— 由本文件原顶层函数归并而来（每个方法对应一个原函数，语义与签名逐字保留）。
 */
export class CancellableModel {
  /**
   ...
   */
  public static cancellableModel(inner: ModelPort, parent: AbortSignal): ModelPort {
    const wrapped: ModelPort = {
      name: inner.name,
      generate: (request: ModelRequest): Promise<ModelOutput> =>
        CancellableModel.withCancel(parent, request.signal, (signal) =>
          inner.generate({ ...request, signal }),
        ),
    };
   ...
   */
  public static async withCancel<T>(
    parent: AbortSignal,
    own: AbortSignal | undefined,
    invoke: (signal: AbortSignal) => Promise<T>,
  ): Promise<T> {
    if (parent.aborted) {
      throw new CancelledError(CancellableModel.reasonOf(parent));
    }
    const link = CancellableModel.linkSignals(parent, own);
    try {
   ...
   * @returns 联动信号与解绑函数
   */
  public static linkSignals(parent: AbortSignal, own: AbortSignal | undefined): LinkedSignal {
