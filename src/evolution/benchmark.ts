...
/**
 * BenchmarkFn 相关纯函数工具（C7 收口：原顶层内部函数迁入）。
 */
export class Benchmark {
   ...
   */
  public static fieldMatch(a: readonly number[], b: readonly number[]): number {
    ...
    let db = 0;
    for (let i = 0; i < n; i++) {
      const x = ArrayAt.at(a, i) - ma;
      const y = ArrayAt.at(b, i) - mb;
      num += x * y;
      da += x * x;
      db += y * y;
    }
   ...
   * 中心化后返回扁平数组。
   */
  public static jointProfile(a: Skill, b: Skill, n: number): number[] {
    const fa = Benchmark.flatten(MoireComposer.capabilityFieldOf(a, n));
    const fb = Benchmark.flatten(MoireComposer.capabilityFieldOf(b, n));
    const prod = fa.map((v, i) => v * ArrayAt.at(fb, i));
   ...
   */
  public static capabilityCoverage(skill: Skill, profile: readonly number[], n: number): number {
    const raw =
      skill.capabilityField !== undefined && skill.capabilityField.length === n * n
        ? [...skill.capabilityField]
        : Benchmark.flatten(MoireComposer.capabilityFieldOf(skill, n));
    return Benchmark.fieldMatch(Benchmark.center(raw), profile as number[]);
  }
   ...
   */
  public static moireEnergy(skill: Skill, n: number, blurR = 2): number {
    const raw =
      skill.capabilityField !== undefined && skill.capabilityField.length === n * n
        ? [...skill.capabilityField]
        : Benchmark.flatten(MoireComposer.capabilityFieldOf(skill, n));
    ...
    let low = 0;
    for (let i = 0; i < len; i++) {
      const d = ArrayAt.at(raw, i) - mean;
      tot += d * d;
      const b = ArrayAt.at(blur, i) - mean;
      low += b * b;
    }
