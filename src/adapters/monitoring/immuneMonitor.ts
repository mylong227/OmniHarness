...
export class ImmuneMonitor implements ImmuneMonitorPort {
  ...
  public train(sample: readonly number[]): void {
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      const x = ArrayAt.at(sample, i);
      const delta = x - ArrayAt.at(this.mean, i);
      this.mean[i] = ArrayAt.at(this.mean, i) + delta / this.n;
      this.M2[i] = ArrayAt.at(this.M2, i) + delta * (x - ArrayAt.at(this.mean, i));
    }
  }
  ...
  public observe(sample: readonly number[]): AnomalyAlert | null {
    ...
    for (let i = 0; i < this.dim; i++) {
      ...
      const std = Math.sqrt(ArrayAt.at(this.M2, i) / Math.max(this.n - 1, 1));
