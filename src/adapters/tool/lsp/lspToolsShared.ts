...
/**
 * LspToolsShared —— 由本文件原顶层函数归并而来（每个方法对应一个原函数，语义与签名逐字保留）。
 */
export class LspToolsShared {
  /** 把文件+1-based 区间渲染成编辑器友好的一行定位（file:line:col）。 */
  public static renderLocation(loc: LspLocation): string {
    const { start } = loc.range;
    return `${loc.uri}:${start.line}:${start.character}`;
  }

  /** 校验并提取 file / line / character（1-based 编辑器坐标）。 */
  public static parseTarget(
    call: ToolCall,
