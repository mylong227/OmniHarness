 ...
 * @returns 衰减因子，取值 (0,1]
 */
export function decayFactor(createdAt: string, nowMs: number, halfLifeDays: number): number {
 ...
 * @returns 已过期返回 true
 */
export function isExpired(fact: MemoryFact, nowMs: number): boolean {
...
export function rankWithDecay(
  ...
  return items
    .filter((it) => !isExpired(it.fact, nowMs))
    .map((it) => ({
      fact: it.fact,
      score: it.score * decayFactor(it.fact.createdAt, nowMs, halfLifeDays),
    }))
