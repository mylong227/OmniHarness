...
async function fetchModelsEndpoint(
  preset: ProviderPreset,
...
async function probeViaChat(preset: ProviderPreset, key: string | undefined): Promise<void> {
  const url =
    preset.adapter === 'anthropic'
  ...
  const body =
    preset.adapter === 'anthropic'
      ? { model: preset.defaultModel, max_tokens: 1, messages: [{ role: 'user', content: 'ping' }] }
      : {
...
export async function probeProvider(
  ...
  try {
    const ids = await fetchModelsEndpoint(preset, preset.needsKey ? key : undefined);
    if (ids !== undefined) {
      return { ...base, ok: true, models: ids, source: 'models-endpoint' };
    }
    await probeViaChat(preset, preset.needsKey ? key : undefined);
