...
/**
 * Bootstrap —— 由本文件原顶层函数归并而来（每个方法对应一个原函数，语义与签名逐字保留）。
 */
export class Bootstrap {
   ...
   * @returns 分位值；空数组返回 0
   */
  public static percentile(sorted: readonly number[], p: number): number {
   ...
   */
  public static bootstrapInterval<T>(
    items: readonly T[],
    ...
    const rand = Bootstrap.mulberry32(opts.seed ?? DEFAULT_BOOTSTRAP_SEED);
    const dist = new Array<number>(rounds);
    for (let b = 0; b < rounds; b++) {
      const resample = new Array<T>(n);
      for (let i = 0; i < n; i++) {
        const k = Math.floor(rand() * n);
        resample[i] = ArrayAt.at(items, k);
      }
    ...
    return {
      mean,
      lo: Bootstrap.percentile(dist, alpha / 2),
      hi: Bootstrap.percentile(dist, 1 - alpha / 2),
      rounds,
    };
  }
}
