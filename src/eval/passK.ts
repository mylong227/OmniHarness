...
/**
 * PassK —— 由本文件原顶层函数归并而来（每个方法对应一个原函数，语义与签名逐字保留）。
 */
export class PassK {
   ...
   * 跨任务取平均。
   */
  public static computePassK(outcomes: readonly TaskSamples[], maxK: number): number[] {
    const result: number[] = [];
    ...
    for (let k = 1; k <= maxK; k++) {
      ...
      for (const runs of outcomes) {
        ...
        const denom = PassK.combination(n, k);
        const num = PassK.combination(n - c, k);
        sum += 1 - num / denom;
      }
  ...
  /** 每任务平均通过率（与采样次数加权）。 */
  public static meanPassRate(outcomes: readonly TaskSamples[]): number {
    let total = 0;
    let samples = 0;
    for (const runs of outcomes) {
      samples += runs.length;
  ...
  public static summarizePassK(outcomes: readonly TaskSamples[], maxK: number): PassKSummary {
    const passAtK = PassK.computePassK(outcomes, maxK);
    return {
      tasks: outcomes.length,
      meanPassRate: PassK.meanPassRate(outcomes),
      passAtK,
   ...
   */
  public static bootstrapPassK(
    outcomes: readonly TaskSamples[],
    maxK: number,
    opts: BootstrapOptions = {},
  ): PassKCIReport {
    const summary = PassK.summarizePassK(outcomes, maxK);
    const passAtKCI: BootstrapResult[] = [];
    for (let k = 1; k <= maxK; k++) {
      passAtKCI.push(
        Bootstrap.bootstrapInterval(outcomes, (rs) => PassK.computePassK(rs, k)[k - 1] ?? 0, opts),
      );
    }
    const meanPassRateCI = Bootstrap.bootstrapInterval(
      outcomes,
      (rs) => PassK.meanPassRate(rs),
      opts,
    );
